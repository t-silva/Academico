...
int main(void)
{
  ...
  int i=0;
  while(nome[i]){
    while(nome[i]!=' ' && nome[i+1]){push(nome[i++],p);}//Se nao coloar o nome[i+1], ele introduz o \0 na pilha,
    while(!isempty(p)) putchar(pop(p));                  //e quebra linha antes do fim da frase.
    i++;
    printf(" ");
    }    
  return 0;
